...
@router.delete("/me")
async def del_user(request: Request, user: APIUser = Depends(get_current_user)):
  pool = request.app.state.pool
  async with pool.acquire() as conn:

  
    target = await conn.fetchrow(
      "DELETE FROM users WHERE id=$1 RETURNING id, username", user.user_id
    )
...
@router.patch("/me")
async def patch_user(body: UpdateUser, request: Request, user: APIUser = Depends(get_current_user)):
  pool = request.app.state.pool
  async with pool.acquire() as conn:
    
    current_user = await conn.fetchrow(
      "SELECT id, username, password FROM users where id=$1", user.user_id
    )
    if not current_user:
      raise HTTPException(status_code=404, detail="User not found")
    ...
    if body.username:
      existing = await conn.fetchrow(
        "SELECT id FROM users WHERE username=$1 AND id <>$2", body.username, user.user_id
      )
      if existing:
...
@router.get("/events")
async def get_user_events(request: Request, user: APIUser = Depends(get_current_user)):
    pool = request.app.state.pool
    async with pool.acquire() as conn:
...
async def reset_user_password(request: Request, user: APIUser = Depends(get_current_user)):
  pool = request.app.state.pool
  async with pool.acquire() as conn:
    
    fetchUser = await conn.fetchrow(
      "SELECT id FROM users WHERE id=$1", user.user_id
    )
    if not fetchUser: 
      raise HTTPException(status_code=404, detail="User not found")
    ...
    await conn.execute(
      "UPDATE users SET password=$1 WHERE id=$2", hashed_password, user.user_id
    )
